...
import { AnimationState, REBEL_ANIMATION_CONFIGS, getRebelAnimationConfig } from '../types/AnimationTypes';
import { REBEL_ASSET_PATHS } from '../utils/Constants';
...
export class EnemyAnimationController {
  ...
  constructor(scene: Scene, assetLoader: AssetLoader, enemyId: string) {
    this.scene = scene;
  ...
  public async initialize(): Promise<{ rootMesh: AbstractMesh; skeleton: Skeleton | null }> {
    ...
    const container = await this.assetLoader.loadAssetContainer(REBEL_ASSET_PATHS.IDLE);

    // Instantiate a unique copy for this enemy
    const instances = container.instantiateModelsToScene(
      (name) => `${this.enemyId}_${name}`,
      false,
      { doNotInstantiate: false }
    );

    if (instances.rootNodes.length === 0) {
      throw new Error('Failed to instantiate enemy model');
    }

    this.rootMesh = instances.rootNodes[0] as AbstractMesh;
    this.skeleton = instances.skeletons[0] || null;

    // Clone the idle animation group for this enemy
    if (instances.animationGroups.length > 0) {
      ...
      const idleConfig = getRebelAnimationConfig(AnimationState.IDLE);
  ...
  private async loadRemainingAnimations(): Promise<void> {
    const animationsToLoad = REBEL_ANIMATION_CONFIGS.filter(
    ...
    for (const config of animationsToLoad) {
      try {
        ...
        if (container.animationGroups.length > 0) {
          // Clone the animation group for this enemy
          const sourceAnimGroup = container.animationGroups[0];
          const clonedAnimGroup = sourceAnimGroup.clone(
            `${this.enemyId}_${config.state}`,
            (oldTarget) => {
              // Retarget to this enemy's skeleton
        ...
        console.error(`Failed to load enemy animation ${config.state}:`, error);
      }
    }
  }
  ...
  public transition(toState: AnimationState, immediate: boolean = false): void {
    ...
    if (!animData) {
      console.warn(`Enemy animation not found for state: ${toState}`);
      // If animation not found and we have a callback, call it anyway
  ...
  private setupAnimationEndHandler(anim: AnimationGroup): void {
    ...
    // Longer timeout to ensure death animation has time to play
  ...
  public dispose(): void {
    ...
    if (this.rootMesh) {
      ...
      for (const child of children) {
        child.dispose(false, false); // Don't dispose materials
      }
      this.rootMesh.dispose(false, false); // Don't dispose materials
    }
